...
import { createRuntimeLifecycle, createRuntimeStopRequestHandler, setControlledByUser } from '@/agent/runtimeLifecycle'
...
export async function runGemini(
    ...
    logger.debug(`[gemini] Starting with options: startedBy=${startedBy}`)
    if (opts.sessionContinuityHandoff) {
        logger.debug('[gemini] Loaded session continuity handoff for Gemini bootstrap')
    }
    ...
    let lifecycle!: ReturnType<typeof createRuntimeLifecycle>
    ...
    session.onUserMessage((message, localId) => {
        ...
        if (continuityInstructions) {
            logger.debug('[gemini] Consuming pending session continuity handoff on the first real user turn')
        }
