...
import { normalizeTerminalInputForHost, resolveShellCommand, TerminalManager } from './TerminalManager'

const originalPlatformDescriptor = Object.getOwnPropertyDescriptor(process, 'platform')
const originalTerminalShell = process.env.VIBY_TERMINAL_SHELL
...
describe('TerminalManager Windows support', () => {
    ...
    beforeEach(() => {
        ...
        process.env.VIBY_TERMINAL_SHELL = 'C:\\Program Files\\PowerShell\\7\\pwsh.exe'
        setPlatform('win32')
    })
    ...
    afterAll(() => {
        ...
        if (originalTerminalShell === undefined) {
            delete process.env.VIBY_TERMINAL_SHELL
        } else {
            process.env.VIBY_TERMINAL_SHELL = originalTerminalShell
        }
    ...
    it('resolves an explicit Windows shell command', () => {
        expect(resolveShellCommand()).toEqual(['C:\\Program Files\\PowerShell\\7\\pwsh.exe'])
    })

    it('normalizes lone line feeds to carriage returns for Windows input', () => {
    ...
    it('opens a Windows PTY instead of rejecting the request', () => {
        ...
        globalWithBun.Bun = { spawn: spawnMock }
        const ready: unknown[] = []
        ...
        expect(spawnMock).toHaveBeenCalledWith(
            ...
            expect.objectContaining({
                ...
                terminal: expect.objectContaining({ cols: 120, rows: 30 }),
            })
        )
