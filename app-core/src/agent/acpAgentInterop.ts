...
export function toAcpMcpServers(config: Record<string, { command: string; args: string[] }>): McpServerStdio[] {
...
export function forwardAcpAgentMessage(message: AgentMessage, options: ForwardAcpAgentMessageOptions): void {
    ...
    switch (message.type) {
        case 'text':
            ...
            return
        case 'reasoning':
            options.addMessage(`[Thinking] ${message.text}`, 'status')
            return
        case 'tool_call':
