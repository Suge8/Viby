...
export function setControlledByUser(session: RuntimeSessionClient, controlledByUser: boolean): void {
...
export function createRuntimeStopRequestHandler(options: RuntimeStopRequestHandlerOptions): () => Promise<void> {
    return async () => {
        try {
            ...
            logger.debug('[runtime-lifecycle] Runtime stop owner failed; falling back to lifecycle cleanup', error)
        }
