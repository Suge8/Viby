...
describe('internalSession', () => {
    ...
    it('accepts pi as an internal agent flavor', () => {
        expect(
            parseInternalSessionArgs(['--agent', 'pi', '--started-by', 'app-core', '--viby-session-id', 'session-pi'])
        ).toEqual(
            expect.objectContaining({
                agent: 'pi',
                startedBy: 'app-core',
    ...
    it('loads a valid driver-switch handoff and forwards it to Codex bootstrap', async () => {
        ...
        expect(harness.loggerDebug).toHaveBeenCalledWith(
            expect.stringContaining('[internal-session] codex load-provider')
        )
        expect(harness.loggerDebug).toHaveBeenCalledWith(
            expect.stringContaining('[internal-session] codex run-provider')
        )
    })

    it('forwards continuity handoff into Gemini bootstrap for app-core-managed resume fallback', async () => {
        const handoffFilePath = await createHandoffFile(createValidHandoffPayload())
    ...
    it('forwards provider-native resume tokens for Pi sessions', async () => {
        await internalSessionCommand.run({
            args: [],
            commandArgs: ['--agent', 'pi', '--started-by', 'app-core', '--resume-session-id', 'pi-provider-session'],
        })
