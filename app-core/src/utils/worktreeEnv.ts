...
import type { WorktreeInfo } from '@/runtime/session/worktree'
...
function readWorktreeFromEnv(): WorktreeInfo | null {
    const basePath = process.env.VIBY_WORKTREE_BASE_PATH?.trim()
    const branch = process.env.VIBY_WORKTREE_BRANCH?.trim()
    const name = process.env.VIBY_WORKTREE_NAME?.trim()
    const worktreePath = process.env.VIBY_WORKTREE_PATH?.trim()
    const createdAtRaw = process.env.VIBY_WORKTREE_CREATED_AT?.trim()
