...
describe('extractErrorInfo', () => {
    it('extracts serverProtocolVersion from axios-style response header', () => {
        const error = {
            ...
            response: {
                status: 400,
                headers: { 'x-viby-protocol-version': '2' },
    ...
    it('extracts serverProtocolVersion from direct property (apiValidationError)', () => {
        const error = new Error('Invalid /runtime/machines response')
        ;(error as unknown as Record<string, unknown>).serverProtocolVersion = 1
        const info = extractErrorInfo(error)
        expect(info.serverProtocolVersion).toBe(1)
        expect(info.message).toBe('Invalid /runtime/machines response')
    })
    ...
    it('prefers direct property over header', () => {
        const error = Object.assign(new Error('test'), {
            ...
            response: {
                status: 200,
                headers: { 'x-viby-protocol-version': '5' },
                data: {},
            },
        })
        const info = extractErrorInfo(error)
    ...
    it('handles non-numeric protocol header gracefully', () => {
        const error = {
            ...
            response: {
                status: 200,
                headers: { 'x-viby-protocol-version': 'abc' },
                data: {},
            },
        }
        const info = extractErrorInfo(error)
...
describe('apiValidationError', () => {
    it('creates error with serverProtocolVersion from response header', () => {
        const fakeResponse = {
            headers: { 'x-viby-protocol-version': '1' },
        }
        const err = apiValidationError('Invalid /runtime/machines response', fakeResponse as any)
        expect(err.message).toBe('Invalid /runtime/machines response')
    ...
    it('creates error without serverProtocolVersion when header missing', () => {
        ...
        const err = apiValidationError('Invalid /runtime/sessions response', fakeResponse as any)
        expect(err.message).toBe('Invalid /runtime/sessions response')
    ...
    it('round-trips through extractErrorInfo', () => {
        ...
        expect(info.serverProtocolVersion).toBe(2)
        expect(info.messageLower).toContain('invalid /runtime/')
    })
})
