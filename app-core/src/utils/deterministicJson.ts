...
type DeterministicJsonPrimitive = null | boolean | number | string
interface DeterministicJsonArray extends Array<DeterministicJsonValue> {}
interface DeterministicJsonObject {
    [key: string]: DeterministicJsonValue
}
type DeterministicJsonValue = DeterministicJsonPrimitive | DeterministicJsonArray | DeterministicJsonObject
 ...
 */
export function deterministicStringify(obj: unknown, options: DeterministicJsonOptions = {}): string {
    const { undefinedBehavior = 'omit', sortArrays = false, replacer, includeSymbols = false } = options

    const seen = new WeakSet()

    function processValue(value: unknown, key?: string): DeterministicJsonValue | undefined {
        ...
        if (value.constructor === Object || value.constructor === undefined) {
            const processed: DeterministicJsonObject = {}
            const objectValue = value as Record<string, unknown>
            const keys = Object.keys(objectValue).sort()

            for (const k of keys) {
                const processedValue = processValue(objectValue[k], k)
