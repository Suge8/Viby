...
function killProcessWindows(pid: number, force: boolean): boolean {
    ...
    try {
        const result = spawn.sync('taskkill', args, { stdio: 'pipe', windowsHide: true })
        if (result.error) {
            return false
        }
...
async function waitForProcessToDie(pid: number, force: boolean): Promise<void> {
    // Arbitrary external PIDs have no Node exit event; bounded polling is the process-kill contract.
    const maxWait = 2000
