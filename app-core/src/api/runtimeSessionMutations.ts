...
import type { MetadataUpdateOptions } from './runtimeSessionState'
import { createWritableSessionMetadataSnapshot, stripLifecycleMetadataFields } from './runtimeSessionState'
...
export async function updateMetadataAndWait(
    ...
    await context.metadataLock.inLock(async () => {
        await backoff(async () => {
            ...
            const answer = await context.channel.request('update-metadata', {
        ...
        () =>
            context.agentStateLock.inLock(async () => {
                await backoff(async () => {
                    ...
                    const answer = await context.channel.request('update-state', {
                        sid: context.sessionId,
