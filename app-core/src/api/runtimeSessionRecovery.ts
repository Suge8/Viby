...
import { isExternalSessionUserMessage, readObservedAutoSummary } from './runtimeSessionState'
import type { AgentState, Metadata, UserMessage } from './types'
import {
    AgentStateSchema,
    MetadataSchema,
    type RuntimeSessionRecoveryResponse,
...
type RecoverSessionStateOptions = {
    state: RecoveryState
    fetchPage: (afterSeq: number) => Promise<RuntimeSessionRecoveryResponse>
...
export async function fetchSessionRecoveryPage(
    ...
    const response = await axios.get(`${apiUrl}/runtime/sessions/${encodeURIComponent(sessionId)}/recovery`, {
        ...requestConfig,
        params: {
            afterSeq,
    ...
    const result = RuntimeSessionRecoveryResponseSchema.safeParse(response.data)
    if (!result.success) {
        throw apiValidationError('Invalid /runtime/sessions/:id/recovery response', response)
    }
...
export function applyRecoveredSessionSnapshot(
    state: RecoveryState,
    session: RuntimeSessionRecoveryResponse['session']
): void {
