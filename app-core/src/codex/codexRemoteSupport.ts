...
export const APP_CORE_RESUME_WARMUP_MAX_ATTEMPTS = 3
const APP_CORE_RESUME_WARMUP_RETRY_BASE_DELAY_MS = 250
...
export function requiresSynchronousResumeWarmup(session: CodexSession): boolean {
    return session.startedBy === 'app-core' && typeof session.sessionId === 'string'
}

export function getResumeWarmupRetryDelayMs(attempt: number): number {
    return attempt * APP_CORE_RESUME_WARMUP_RETRY_BASE_DELAY_MS
}
