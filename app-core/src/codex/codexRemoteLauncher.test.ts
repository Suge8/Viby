...
function createSessionStub(modes: EnhancedMode[] = [createMode()], options: { autoCloseQueue?: boolean } = {}) {
    ...
    const session = {
        ...
        startedBy: 'terminal' as 'app-core' | 'terminal',
        thinking: false,
        getPermissionMode() {
...
describe('codexRemoteLauncher', () => {
    ...
    it('retries resume warmup during app-core-managed startup until the old thread reattaches', async () => {
        harness.resumeThreadFailuresRemaining = 2

        const { session, foundSessionIds } = createSessionStub()
        session.startedBy = 'app-core'
