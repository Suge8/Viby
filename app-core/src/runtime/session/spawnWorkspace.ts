        ...
        workspace: {
            ...
            maybeCleanupWorktree: async (reason: string, pid?: number | null) => {
                ...
                if (pid && isProcessAlive(pid)) {
                    logger.debug(`[RuntimeSupervisor] Skipping worktree cleanup after ${reason}; child still running`, {
                        pid,
                ...
                if (!result.ok) {
                    logger.debug(
                        `[RuntimeSupervisor] Failed to remove worktree ${worktreeInfo.worktreePath}: ${result.error}`
                    )
                }
            },
        },
