...
export function isAppCoreManagedSession(session: TrackedSession): boolean {
    return session.startedBy === APP_CORE_MANAGED_STARTED_BY && Boolean(session.childProcess)
}

export async function stopTrackedSessionProcess(session: TrackedSession): Promise<boolean> {
    if (isAppCoreManagedSession(session) && session.childProcess) {
...
export async function stopAppCoreManagedSessions(
    sessions: Iterable<TrackedSession>
): Promise<StopManagedSessionsResult> {
    const managedSessions = Array.from(sessions).filter(isAppCoreManagedSession)
