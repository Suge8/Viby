 ...
 * - VIBY_CLAUDE_PATH: Force a specific path to claude executable
 */
export function getDefaultClaudeCodePath(): string {
    // Allow explicit override via env var
    if (process.env.VIBY_CLAUDE_PATH) {
        logger.debug(`[Claude SDK] Using VIBY_CLAUDE_PATH: ${process.env.VIBY_CLAUDE_PATH}`)
        return process.env.VIBY_CLAUDE_PATH
    }

    // Find global claude
    const globalPath = findGlobalClaudePath()
    if (!globalPath) {
        throw new Error('Claude Code CLI not found on PATH. Install Claude Code or set VIBY_CLAUDE_PATH.')
    }
    return globalPath
}
