...
const artifactDir = join(repoRoot, '.artifacts/audit/governance')
const scanExtensions = new Set(['.ts', '.tsx', '.js', '.jsx', '.css'])
const rootByModule = {
    web: 'web/src',
    hub: 'hub/src',
    runtime: 'app-core/src',
    desktop: 'desktop/src',
    pairing: 'pairing/src',
    shared: 'shared/src',
} as const
...
function main(): void {
    ...
    if (result.violations.length > 0) {
        console.warn('[audit] governance found issues:')
    ...
    console.log('[audit] governance passed')
}
