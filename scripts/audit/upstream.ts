...
const artifactDir = join(repoRoot, '.artifacts/audit/upstream')
...
function main(): void {
    ...
    console.log(`[audit] upstream: ${snapshot.status}`)
    console.log(`[audit] ledger: ${snapshot.ledgerPath}`)
    console.log(`[audit] pending upstream commits: ${snapshot.auditCursor.pendingCommitCount ?? 'unknown'}`)
    if (snapshot.warnings.length > 0) {
        for (const warning of snapshot.warnings) {
