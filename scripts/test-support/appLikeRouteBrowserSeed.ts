...
import { getJson, postJson } from './browserIsolatedApp'
import { type StoredTranscriptSeedInput, seedStoredSessionMessages } from './transcriptSeed'
...
export async function seedRuntimeAndSessions(options: {
    ...
    for (const [tag, name, workspace] of [
        ...
        const response = await postJson<SeededSessionResponse>(`${options.hubUrl}/runtime/sessions`, {
...
async function registerSmokeRuntime(
    ...
    await postJson(`${options.hubUrl}/runtime/machines`, {
        headers: { authorization: `Bearer ${options.hubOwnerToken}` },
        body: {
