...
export function resolveChromeExecutablePath(): string {
    ...
    const bundledChrome = existsSync(browserRoot)
    ...
    const executablePath = bundledChrome ?? systemBrowser

    if (!executablePath) {
        throw new Error('failed to locate a Chrome-compatible browser for Playwright')
    }
