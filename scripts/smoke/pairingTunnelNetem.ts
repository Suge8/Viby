...
import { type DockerRuntimeLease, ensureDockerRuntime } from '../test-support/colimaDockerRuntime'
...
async function main(): Promise<void> {
    ...
    if (!dockerRuntime) {
        if (process.env.VIBY_PAIRING_NETEM_SKIP_IF_UNAVAILABLE === '1') {
            console.log('[smoke] pairing netem smoke skipped: docker daemon unavailable')
            return
        }
    ...
    try {
        ...
        try {
            ...
            if (process.env.VIBY_PAIRING_NETEM_CLEAN_ARTIFACTS === '1') {
                ...
                console.log(`[smoke] pairing netem artifacts kept at ${workDir}`)
            }
        } finally {
