...
import { seedRuntimeAndSessions } from '../test-support/appLikeRouteBrowserSeed'
import {
    type IsolatedBrowserApp,
    launchObservedMobileBrowser,
    startIsolatedBrowserApp,
    stopProcess,
} from '../test-support/browserIsolatedApp'
import {
...
} from '../test-support/transcriptSeed'

const scriptDir = fileURLToPath(new URL('.', import.meta.url))
const repoRoot = resolve(scriptDir, '../..')
const timestamp = new Date().toISOString().replaceAll(':', '-')
const outputDir = resolve(repoRoot, `web/.artifacts/smoke/${timestamp}-chat-bottom-bounce-repro`)
...
async function main(): Promise<void> {
    ...
    try {
        ...
        await context.addInitScript(
            ({ hubUrl: targetHubUrl, token: targetAccessToken }) => {
                window.localStorage.setItem('viby_hub_url', targetHubUrl)
                window.localStorage.setItem(`viby_access_token::${targetHubUrl}`, targetAccessToken)
            },
            { hubUrl, token: accessToken }
        )
