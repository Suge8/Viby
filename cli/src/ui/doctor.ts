 ...
 * for viby CLI including configuration, runner status, logs, and links
 */
...
import { findAllVibyProcesses, findRunawayVibyProcesses } from '@/runner/doctor'
...
export async function runDoctorCommand(filter?: 'all' | 'runner'): Promise<void> {
    ...
    if (filter === 'all') {
        ...
        console.log(`viby CLI Version: ${chalk.green(packageJson.version)}`)
        ...
        console.log(`viby Home: ${chalk.blue(configuration.vibyHomeDir)}`)
    ...
    try {
        ...
        if (allProcesses.length > 0) {
            ...
            console.log(chalk.red('❌ No viby processes found'))
        }

        if (filter === 'all' && allProcesses.length > 1) {
            // More than just current process
            console.log(chalk.bold('\n💡 Process Management'))
            console.log(chalk.gray('To clean up runaway processes: viby doctor clean'))
        }
    } catch (error) {
