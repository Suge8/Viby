...
type MemberPromptContractInput = {
    prototype: MemberRolePrototype
    roleId?: string
    roleName?: string
...
function buildRoleIdentityLine(input: MemberPromptContractInput): string | undefined {
    ...
    if (roleId && roleName) {
        return `Your authoritative role definition is id "${roleId}" named "${roleName}".`
    }

    if (roleId) {
        return `Your authoritative role definition id is "${roleId}".`
    }

    return `Your authoritative role definition name is "${roleName}".`
}

function buildPromptExtensionSegment(input: MemberPromptContractInput): string | undefined {
    const promptExtension = input.promptExtension?.trim()
    if (!promptExtension) {
        return undefined
    }

    return trimIdent(`
        Authoritative role prompt extension:
        ${promptExtension}

        Treat this as an append-only specialization on top of the "${input.prototype}" prototype.
        Do not override the manager contract, system instructions, or tool contract.
    `)
}
...
export function buildManagerPromptContract(): string {
    return trimIdent(`
        ...
        Use the team_get_snapshot tool and the returned compactBrief as the authoritative
        orchestration brief for the current team project.
        Treat compactBrief.wakeReasons as typed replan signals and
        compactBrief.nextActions as the current execution queue.
        Treat compactBrief.staffing as the authoritative staffing and seat-budget policy view.
...
export function buildMemberPromptContract(
    input: MemberRolePrototype | MemberPromptContractInput
): string {
    const resolvedInput = typeof input === 'string'
        ? { prototype: input }
        : input
    const promptSegments = [trimIdent(`
        You are a Viby team member, not the manager.
        Your role prototype is "${resolvedInput.prototype}".
        ${MEMBER_ROLE_FOCUS[resolvedInput.prototype]}
    ...
    `)]
    const roleIdentityLine = buildRoleIdentityLine(resolvedInput)
    if (roleIdentityLine) {
        promptSegments.push(roleIdentityLine)
    }

    const promptExtensionSegment = buildPromptExtensionSegment(resolvedInput)
    if (promptExtensionSegment) {
        promptSegments.push(promptExtensionSegment)
    }
...
export function resolveTeamRolePromptContract(
    teamContext?: Pick<SessionTeamContext, 'sessionRole' | 'memberRole' | 'memberRoleId' | 'memberRoleName' | 'memberRolePromptExtension'>
): string | undefined {
    if (!teamContext) {
        return undefined
    }
    ...
    return buildMemberPromptContract({
        prototype: memberRole,
        roleId: teamContext.memberRoleId,
        roleName: teamContext.memberRoleName,
        promptExtension: teamContext.memberRolePromptExtension
    })
}
...
export function prependPromptInstructionsToMessage(
    message: string,
    instructions?: string
): string {
