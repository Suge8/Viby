...
/**
 * Filter out 'resume' subcommand which is managed internally by Viby.
...
export function filterResumeSubcommand(args: string[]): string[] {
    ...
    if (args.length > 1 && !args[1].startsWith('-')) {
        logger.debug(`[CursorLocal] Filtered 'resume ${args[1]}' - session managed by Viby`);
        return args.slice(2);
    }

    logger.debug(`[CursorLocal] Filtered 'resume' - session managed by Viby`);
    return args.slice(1);
}
