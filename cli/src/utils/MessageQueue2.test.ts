...
describe('MessageQueue2', () => {
    ...
    it('should emit consumed local IDs when a batch is dequeued', async () => {
        ...
        queue.push('third', { type: 'B' }, 'local-3')

        await queue.waitForMessagesAndGetAsString()
        await queue.waitForMessagesAndGetAsString()

        expect(consumed).toEqual([['local-1', 'local-2'], ['local-3']])
    })
    ...
    it('should emit canceled local IDs when clearing queued messages', () => {
        ...
        queue.push('first', { type: 'A' }, 'local-1')
        queue.push('second', { type: 'A' }, 'local-2')
        queue.pushIsolateAndClear('replacement', { type: 'A' }, 'local-3')
        queue.reset()

        expect(canceled).toEqual([['local-1', 'local-2'], ['local-3']])
    })
    ...
    it('should remove queued messages by local ID without emitting a cancellation loopback', async () => {
        ...
        expect(queue.removeByLocalIds(['local-1'])).toEqual(['local-1'])
        expect(canceled).toEqual([])

        const batch = await queue.waitForMessagesAndGetAsString()
        expect(batch?.message).toBe('second')
    })
})
