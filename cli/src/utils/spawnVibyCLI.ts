/**
 * Cross-platform VIBY CLI spawning utility
 *
 * ## Background
 *
 * VIBY CLI runs in two modes:
 ...
 *
 * This utility handles spawning VIBY CLI subprocesses (for runner processes)
...
export interface VibyCliCommand {
  command: string;
  args: string[];
}
...
export function getVibyCliCommand(args: string[]): VibyCliCommand {
  ...
  if (isBunRuntime) {
    ...
    // keeps working even when external tools launch VIBY from another workspace.
    return {
      command: process.execPath,
...
export function spawnVibyCLI(args: string[], options: SpawnOptions = {}): ChildProcess {
  ...
  // bypassing the 'viby' wrapper that would normally be found in the shell's PATH.
  // However, we log it as 'viby' here because other engineers are typically looking
  // for when "viby" was started and don't care about the underlying node process
  // details and flags we use to achieve the same result.
  const fullCommand = `viby ${args.join(' ')}`;
  logger.debug(`[SPAWN VIBY CLI] Spawning: ${fullCommand} in ${directory}`);
  
  const { command: spawnCommand, args: spawnArgs } = getVibyCliCommand(args);
  ...
  if (!isBunCompiled()) {
    ...
    if (entrypoint && !existsSync(entrypoint)) {
      ...
      logger.debug(`[SPAWN VIBY CLI] ${errorMessage}`);
