...
describe('spawnVibyCLI windowsHide behavior', () => {
  beforeAll(() => {
  ...
  it('sets windowsHide=true when platform is win32 and detached=true', async () => {
    setPlatform('win32');
    const { spawnVibyCLI } = await import('./spawnVibyCLI');

    spawnVibyCLI(['runner', 'start-sync'], {
      detached: true,
      stdio: 'ignore'
    });
  ...
  it('forces Bun child processes to run with the cli project root as cwd', async () => {
    const { getVibyCliCommand } = await import('./spawnVibyCLI');

    const command = getVibyCliCommand(['mcp', '--url', 'http://127.0.0.1:1234/']);
  ...
  it('prefers the explicit child cwd over an inherited VIBY_INVOKED_CWD', async () => {
    ...
    spawnVibyCLI(['runner', 'start-sync'], {
      cwd: childCwd,
      env: {
        VIBY_INVOKED_CWD: inheritedInvokedCwd
      },
      stdio: 'ignore'
    });
  ...
  it('keeps an existing absolute VIBY_INVOKED_CWD when no child cwd is provided', async () => {
    ...
    const options = getSpawnOptionsOrThrow();
    expect(options.env?.VIBY_INVOKED_CWD).toBe(inheritedInvokedCwd);
  });
});
