/**
 * Minimal persistence functions for VIBY CLI
 *
 * Handles settings, encryption key, and runner state storage in ~/.viby/ (or VIBY_HOME override)
 */
...
function reportPersistenceError(message: string, error: unknown): void {
    ...
    process.stderr.write(`[PERSISTENCE] ${message}${detail ? ` ${detail}` : ''}\n`)
}
...
export async function readRunnerState(): Promise<RunnerLocallyPersistedState | null> {
    try {
        ...
        reportPersistenceError(`Runner state file corrupted: ${configuration.runnerStateFile}`, error)
        return null
    }
}
