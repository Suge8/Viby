...
/**
 * Find all VIBY CLI processes (including current process)
 */
export async function findAllVibyProcesses(): Promise<Array<{ pid: number, command: string, type: string }>> {
  try {
    ...
    for (const proc of processes) {
      ...
      // Check if it's a VIBY process
      const isVibyBinary = name === 'viby' || name === 'viby.exe' || /\bviby(\.exe)?\b/.test(cmd);
      // Dev mode: running via bun/node with src/index.ts (production uses compiled binary)
      const isDevMode = cmd.includes('src/index.ts');
      const isViby = name.includes('viby') ||
                      name === 'node' && cmd.includes('viby-cli') ||
                      isVibyBinary ||
                      isDevMode;
...
/**
 * Find all runaway VIBY CLI processes that should be killed
 */
export async function findRunawayVibyProcesses(): Promise<Array<{ pid: number, command: string }>> {
  const allProcesses = await findAllVibyProcesses();
...
/**
 * Kill all runaway VIBY CLI processes
 */
export async function killRunawayVibyProcesses(): Promise<{ killed: number, errors: Array<{ pid: number, error: string }> }> {
  const runawayProcesses = await findRunawayVibyProcesses();
