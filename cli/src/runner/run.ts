...
export async function startRunner(): Promise<void> {
  ...
  let requestShutdown: (source: 'viby-app' | 'viby-cli' | 'os-signal' | 'exception', errorMessage?: string) => void;
  let resolvesWhenShutdownRequested = new Promise<({ source: 'viby-app' | 'viby-cli' | 'os-signal' | 'exception', errorMessage?: string })>((resolve) => {
  ...
  try {
    ...
    // Handle webhook from VIBY session reporting itself
    ...
    const spawnSession = async (options: SpawnSessionOptions): Promise<SpawnSessionResult> => {
      ...
      let vibyProcess: ReturnType<typeof spawnVibyCLI> | null = null;
      ...
      try {
        ...
        // Wait for webhook to populate session with vibySessionId
        ...
        const spawnResult = await new Promise<SpawnSessionResult>((resolve) => {
          ...
          pidToAwaiter.set(pid, (completedSession) => {
            ...
            pidToErrorAwaiter.delete(pid);
            logger.debug(`[RUNNER RUN] Session ${completedSession.vibySessionId} fully spawned with webhook`);
            resolve({
              type: 'success',
    ...
    // Setup signal handlers
    const cleanupAndShutdown = async (source: 'viby-app' | 'viby-cli' | 'os-signal' | 'exception', errorMessage?: string) => {
      ...
      if (managedSessionStopResult.failedPids.length > 0) {
        logger.debug('[RUNNER RUN] Failed to stop some runner-managed sessions before shutdown', managedSessionStopResult.failedPids);
      }
