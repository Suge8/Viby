 ...
 * On invocation it forwards the tool call to an existing VIBY HTTP MCP server
...
export async function runVibyMcpStdioBridge(argv: string[]): Promise<void> {
  try {
    ...
    const baseUrl = urlFromArgs || process.env.VIBY_HTTP_MCP_URL || '';
    ...
    try {
      process.stderr.write(`[viby-mcp] Fatal: ${err instanceof Error ? err.message : String(err)}\n`);
    } finally {
      process.exit(1);
    }
  }
