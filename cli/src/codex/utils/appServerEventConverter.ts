...
function withTurnId(event: ConvertedEvent, turnId: string | null): ConvertedEvent {
    if (!turnId) {
        return event;
    }
...
function extractThreadId(params: Record<string, unknown>): string | null {
    ...
    if (thread) {
        return asString(thread.threadId ?? thread.thread_id ?? thread.id);
    }
...
export class AppServerEventConverter {
    ...
    handleNotification(method: string, params: unknown): ConvertedEvent[] {
        ...
        if (method === 'thread/started' || method === 'thread/resumed' || method === 'thread/compacted') {
        ...
        if (method === 'turn/started') {
            ...
            events.push(withTurnId({ type: 'task_started' }, turnId));
            return events;
        }
        ...
        if (method === 'turn/completed') {
            ...
            if (status === 'interrupted' || status === 'cancelled' || status === 'canceled') {
                events.push(withTurnId({ type: 'turn_aborted' }, turnId));
                return events;
            }

            if (status === 'failed' || status === 'error') {
                events.push(withTurnId({
                    type: 'task_failed',
            ...
            events.push(withTurnId({ type: 'task_complete' }, turnId));
            return events;
        }
        ...
        if (method === 'item/agentMessage/delta') {
            ...
            if (itemId && delta) {
                ...
                events.push(withTurnId({ type: 'agent_message_delta', item_id: itemId, delta }, turnId));
            }
            return events;
        }
        ...
        if (method === 'item/reasoning/textDelta' || method === 'item/reasoning/summaryTextDelta') {
            ...
            if (delta) {
                ...
                events.push(withTurnId({ type: 'agent_reasoning_delta', delta }, turnId));
            }
            return events;
        }
        ...
        if (method === 'item/reasoning/summaryPartAdded') {
            ...
            events.push(withTurnId({ type: 'agent_reasoning_section_break' }, turnId));
            return events;
        }
        ...
        if (method === 'item/started' || method === 'item/completed') {
            ...
            if (itemType === 'agentmessage') {
                if (method === 'item/completed') {
                    ...
                    if (text) {
                        events.push(withTurnId({ type: 'agent_message', item_id: itemId, message: text }, turnId));
            ...
            if (itemType === 'reasoning') {
                if (method === 'item/completed') {
                    ...
                    if (text) {
                        events.push(withTurnId({ type: 'agent_reasoning', text }, turnId));
