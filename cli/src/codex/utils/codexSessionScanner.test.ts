...
describe('codexSessionScanner', () => {
    ...
    it('limits session scan to dates within the start window', async () => {
        ...
        const baseLines = [
            JSON.stringify({ type: 'session_meta', payload: { id: matchingSessionId, cwd: '/data/github/viby/viby', timestamp: '2025-12-22T00:00:30.000Z' } }),
            JSON.stringify({ type: 'event_msg', payload: { type: 'agent_message', message: 'hello' } })
        ];
        await writeFile(matchingFile, baseLines.join('\n') + '\n');
        await writeFile(
            outsideFile,
            JSON.stringify({ type: 'session_meta', payload: { id: outsideSessionId, cwd: '/data/github/viby/viby', timestamp: '2025-12-20T00:00:00.000Z' } }) + '\n'
        );

        scanner = await createCodexSessionScanner({
            sessionId: null,
            cwd: '/data/github/viby/viby',
    ...
    it('adopts a reused older session file when fresh matching activity appears after startup', async () => {
        ...
        const targetCwd = '/data/github/viby/viby';
