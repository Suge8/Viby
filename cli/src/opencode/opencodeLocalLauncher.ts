...
function resolveOpencodeConfigDir(session: OpencodeSession): string {
    ...
    return join(configuration.vibyHomeDir, 'tmp', 'opencode', session.client.sessionId, '.opencode');
}
...
export async function opencodeLocalLauncher(
    ...
    // Start the Viby MCP server for change_title support.
    let vibyServer: { url: string; stop: () => void } | null = null;
    let opencodeConfigPath: string | null = null;
    try {
        const bridge = await buildVibyMcpBridge(session.client);
        vibyServer = bridge.server;
    ...
    const launcher = new BaseLocalLauncher({
        ...
        launch: async (abortSignal) => {
            ...
            env.VIBY_OPENCODE_HOOK_URL = hookUrl;
            env.VIBY_OPENCODE_HOOK_TOKEN = opts.hookServer.token;
    ...
    try {
        ...
        if (vibyServer) {
            vibyServer.stop();
            logger.debug('[opencode-local]: Stopped Viby MCP server');
        }
    }
}
