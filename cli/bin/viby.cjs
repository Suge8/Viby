...
const RELEASE_URL = 'https://github.com/Suge8/Viby/releases'
...
function getBinaryPath(platformName = platform, archName = arch) {
    ...
    try {
        ...
        const binName = platformName === 'win32' ? 'viby.exe' : 'viby'
...
function reportMissingPlatformPackage(platformName = platform, archName = arch, log = console.error) {
    ...
    log('')
    log(
        `Detected platform ${platformName}-${archName} is supported, but the platform binary package was not installed.`
    )
