...
function replaceOptionalDependencyVersion(content: string, packageName: string, version: string): string {
...
export function syncWorkspaceVersion(version: string): void {
    ...
    syncPackageJsonVersion(join(REPO_ROOT, 'pairing', 'package.json'), version)
