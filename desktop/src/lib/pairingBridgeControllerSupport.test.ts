...
describe('pairingBridgeControllerSupport', () => {
    ...
    it('keeps data channel close in paused recovery before full reconnect', () => {
        ...
        attachPairingDataChannel({
            nextChannel: channel as unknown as RTCDataChannel,
            client: client as never,
            isDisposed: () => false,
            getSuppressTransportClose: () => false,
            setBridgeState: () => undefined,
            stopEventStream,
    ...
    it('prefers ICE restart when the phone returns to a disconnected peer', async () => {
        ...
        expect(tryIceRestart).toHaveBeenCalledWith('手机已回来，正在刷新点对点链路。')
    ...
    it('rebuilds immediately when the phone rejoins after the old data channel closed', async () => {
        ...
        expect(resetOfferState).toHaveBeenCalled()
        expect(scheduleReconnect).toHaveBeenCalledWith('手机已回来，正在重建安全链路。')
        expect(ensureOffer).not.toHaveBeenCalled()
    ...
    it('rebuilds immediately when a new guest transport rejoins while the old transport looks connected', async () => {
        const scheduleReconnect = mock(() => undefined)
        const rebuildTransport = mock(() => undefined)
        const ensureOffer = mock(async () => undefined)
        const resetOfferState = mock(() => undefined)
        ...
        expect(resetOfferState).toHaveBeenCalled()
        expect(rebuildTransport).toHaveBeenCalledWith('手机已回来，正在重建安全链路。')
        expect(scheduleReconnect).not.toHaveBeenCalled()
        expect(ensureOffer).not.toHaveBeenCalled()
        expect(setBridgeState).not.toHaveBeenCalled()
    })

    it('rebuilds immediately when a new guest transport rejoins a disconnected old peer', async () => {
        const rebuildTransport = mock(() => undefined)
        const tryIceRestart = mock(() => true)
        const ensureOffer = mock(async () => undefined)
        const resetOfferState = mock(() => undefined)
        ...
        expect(resetOfferState).toHaveBeenCalled()
        expect(rebuildTransport).toHaveBeenCalledWith('手机已回来，正在重建安全链路。')
        expect(tryIceRestart).not.toHaveBeenCalled()
        expect(ensureOffer).not.toHaveBeenCalled()
    })
    ...
    it('keeps a healthy transport when the same guest transport only rejoins signaling', async () => {
        ...
        expect(resetOfferState).not.toHaveBeenCalled()
        expect(ensureOffer).not.toHaveBeenCalled()
        expect(setBridgeState).toHaveBeenCalledWith({ phase: 'ready', message: '手机链路已接通。', pairing: snapshot })
    })
    ...
    it('keeps the bridge ready when late state updates arrive after the peer connected', async () => {
        const setBridgeState = mock(() => undefined)
        const snapshot = createSnapshot()
