...
- `public_access_enabled` 是公网 Hub 直连 + pairing/WebRTC/TURN 的唯一 Hub policy；默认开启，关闭时 Hub 必须拒绝公网 Host/Origin/forwarded client 与新 pairing create，但保留 loopback/LAN/Tailscale 等本地访问
...
- session 自然结束、inactive 过期、以及 Hub 冷启动历史修复，必须统一把 `active=false + lifecycleState=running` 归一到 `closed`；禁止把裂脑旧账继续带进下一次启动
