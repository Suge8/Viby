...
function getVibyHome(): string {
    return process.env.VIBY_HOME
...
function getTunwgPath(): string {
    ...
    if (isBunCompiled()) {
        const vibyHome = getVibyHome()
        const packageJson = require('../../../cli/package.json')
        const runtimePath = join(vibyHome, 'runtime', packageJson.version)
...
export interface TunnelConfig {
    localPort: number
    enabled: boolean
    apiDomain?: string | null  // TUNWG_API - default: relay.viby.run (official relay)
    authKey?: string | null    // TUNWG_AUTH - default: viby
...
export class TunnelManager {
    ...
    private async spawnTunwg(): Promise<string | null> {
        ...
        if (!env.TUNWG_PATH) {
            env.TUNWG_PATH = join(getVibyHome(), 'tunwg')
        }

        if (this.config.apiDomain) {
            env.TUNWG_API = this.config.apiDomain
        }
        env.TUNWG_AUTH = this.config.authKey ?? 'viby'
