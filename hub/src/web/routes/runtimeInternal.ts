...
export function createRuntimeInternalRoutes(getSyncEngine: () => SyncEngine | null): Hono<RuntimeInternalEnv> {
    const app = new Hono<RuntimeInternalEnv>()
    ...
    app.post('/machines', createJsonBodyValidator(createOrLoadMachineSchema), async (c) => {
        ...
        const machine = engine.getOrCreateMachine(body.id, body.metadata, body.runtimeState ?? null)
