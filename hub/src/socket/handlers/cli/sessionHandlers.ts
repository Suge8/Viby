...
type SessionLifecycleMetadataField =
    | 'lifecycleState'
    | 'lifecycleStateSince'
    | 'archivedBy'
    | 'archiveReason'
...
export function registerSessionHandlers(socket: CliSocketWithData, deps: SessionHandlersDeps): void {
    ...
    socket.on('message', (data: unknown) => {
        ...
        if (shouldMessageAdvanceSessionUpdatedAt(activityKind)) {
            store.sessions.touchSessionUpdatedAt(sid, msg.createdAt)
        }
    ...
    socket.on('stream-update', (data: unknown) => {
        ...
        const event = update.kind === 'append'
            ? sessionStreamManager.applyUpdate(sid, {
                kind: 'append',
