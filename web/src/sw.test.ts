...
function createWaitUntilEvent<T extends object>(
    event: T
): T & {
...
describe('service worker push notifications', () => {
    ...
    it('keeps non-critical optional chunks out of the app-shell precache manifest', async () => {
        ...
        ).toEqual(['/assets/index-main.js', '/assets/index-main.css'])
    })

    it('matches non-critical precache asset urls using the shared chunk markers', () => {
    ...
    it('matches only the core syntax and markdown runtime chunks for optional runtime caching', () => {
        expect(isOptionalRuntimeCacheAssetUrl('/assets/MarkdownRenderer-abc123.js')).toBe(true)
        expect(isOptionalRuntimeCacheAssetUrl('/assets/typescript-abc123.js')).toBe(true)
        expect(isOptionalRuntimeCacheAssetUrl('/assets/python-abc123.js')).toBe(true)
        expect(isOptionalRuntimeCacheAssetUrl('/assets/csharp-abc123.js')).toBe(false)
        expect(isOptionalRuntimeCacheAssetUrl('/assets/swift-abc123.js')).toBe(false)
        expect(isOptionalRuntimeCacheAssetUrl('/assets/index-main.js')).toBe(false)
    })
    ...
    it('registers a cache-first runtime route for optional same-origin assets', async () => {
        ...
        const optionalAssetRouteCall = workboxMocks.registerRoute.mock.calls.find((call) => {
            ...
            return matcher({
                ...
                url: new URL('https://app.viby.run/assets/MarkdownRenderer-abc123.js'),
            })
        })
        ...
        expect(
            matcher({
                request: { destination: 'script' } as Request,
                url: new URL('https://app.viby.run/assets/csharp-abc123.js'),
            })
        ).toBe(false)
        expect(
            matcher({
        ...
        const strategy = optionalAssetRouteCall?.[1] as {
            options?: { plugins?: Array<{ type?: string; options?: { statuses?: number[] } }> }
        }
        ...
        const cacheablePlugin = strategy.options?.plugins?.[0] as {
            cacheWillUpdate: (options: { response: Response }) => Promise<Response | null>
        }
        await expect(
            cacheablePlugin.cacheWillUpdate({ response: new Response('ok', { status: 200 }) })
        ).resolves.toBeInstanceOf(Response)
        await expect(
            cacheablePlugin.cacheWillUpdate({ response: new Response('missing', { status: 404 }) })
        ).resolves.toBeNull()
    })
    ...
    it('only caches successful same-origin API responses in network-first routes', async () => {
        ...
        const strategy = apiRouteCall?.[1] as {
            options?: { plugins?: Array<{ type?: string; options?: { statuses?: number[] } }> }
        }
        expect(strategy.options?.plugins).toEqual([
            expect.objectContaining({
        ...
        await expect(
            cacheablePlugin.cacheWillUpdate({ response: new Response('{}', { status: 200 }) })
        ).resolves.toBeInstanceOf(Response)
        await expect(
            cacheablePlugin.cacheWillUpdate({ response: new Response('{}', { status: 500 }) })
        ).resolves.toBeNull()
    })
    ...
    it('allows opaque CDN responses for socket.io runtime caching', async () => {
        ...
        const strategy = cdnRouteCall?.[1] as {
            options?: { plugins?: Array<{ type?: string; options?: { statuses?: number[] } }> }
        }
        ...
        await expect(cacheablePlugin.cacheWillUpdate({ response: { status: 0 } as Response })).resolves.toEqual({
            status: 0,
        })
    })
