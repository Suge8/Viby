...
export function LazyRainbowText(props: { text: string }) {
    ...
    const shouldUseMarkdown = isLikelyMarkdownText(text)
    ...
    const plainTextContent = hasSpecialWord && hasBeenVisible
        ? processTextForRainbow(text)
        : text

    if (!shouldUseMarkdown) {
        return (
            <div ref={ref} className="whitespace-pre-wrap break-words text-base">
                {plainTextContent}
            </div>
        )
    }
    ...
    return (
        <div ref={ref}>
            <Suspense fallback={<div className="whitespace-pre-wrap break-words text-base">{plainTextContent}</div>}>
