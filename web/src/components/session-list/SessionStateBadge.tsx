...
const SESSION_STATE_BADGE_CLASS_NAME =
    'ds-session-state-badge inline-flex min-h-7 items-center gap-1.5 rounded-full px-2.5 py-1 font-semibold'
...
type SessionStateBadgeProps = {
    presentation: Pick<SessionStatePresentation, 'badgeClassName' | 'badgeIconClassName' | 'badgeIconName' | 'labelKey'>
}
    ...
    return (
        <span className={`${SESSION_STATE_BADGE_CLASS_NAME} ${props.presentation.badgeClassName}`}>
            <span className={SESSION_STATE_ICON_FRAME_CLASS_NAME}>
                <span className={`relative ${props.presentation.badgeIconClassName}`}>{icon}</span>
            </span>
