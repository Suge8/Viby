...
export function usePiLaunchConfig(options: UsePiLaunchConfigOptions): PiLaunchConfigState {
    ...
    useEffect(() => {
        if (options.agent !== 'pi' || !options.machineId || !options.directory) {
            setState({ config: null, error: null })
            return
        }

        const cacheKey = `${options.machineId}:${options.directory}`
        ...
        void options.api.resolveAgentLaunchConfig(options.machineId, {
            agent: 'pi',
    ...
    }, [options.agent, options.api, options.directory, options.machineId, options.t])
