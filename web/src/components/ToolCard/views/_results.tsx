...
function extractTextFromResult(result: unknown, depth: number = 0): string | null {
    if (depth > TOOL_RESULT_MAX_DEPTH) return null
...
function renderText(text: string, opts: { mode: 'plain' | 'markdown' | 'code'; language?: string } = { mode: 'plain' }) {
    ...
    if (looksLikeHtml(text) || looksLikeJson(text)) {
        return (
            <CodeBlock
                code={text}
                language={looksLikeJson(text) ? 'json' : 'html'}
                highlight="never"
            />
        )
    }
...
const MutationResultView: ToolViewComponent = (props: ToolViewProps) => {
    ...
    if (result === undefined || result === null) {
        if (state === 'completed') {
            return <div className="text-sm text-[var(--app-hint)]">{DONE_TEXT}</div>
        }
    ...
    return (
        <>
            <div className="text-sm text-[var(--app-hint)]">
                {state === 'completed' ? DONE_TEXT : NO_OUTPUT_TEXT}
            </div>
            <RawJsonDevOnly value={result} />
        </>
    )
