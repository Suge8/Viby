...
function PermissionRowButton(props: {
    ...
    const base = 'w-full rounded-md px-2 py-2 text-sm text-left transition-colors disabled:pointer-events-none disabled:opacity-50 hover:bg-[var(--app-subtle-bg)] [&>[data-button-content]]:w-full [&>[data-button-content]]:justify-between'
...
export function PermissionFooter(props: {
    ...
    const deniedPreset = getNoticePreset('toolPermissionDenied', t)
    const canceledPreset = getNoticePreset('toolCanceled', t)
    ...
    if (!isPending) {
        ...
        return (
            <div className="mt-2">
                <InlineNotice
                    tone={permission.status === 'canceled' ? canceledPreset.tone : deniedPreset.tone}
                    title={permission.status === 'canceled' ? canceledPreset.title : deniedPreset.title}
