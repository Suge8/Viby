...
const CHUNK_BUDGETS = [
  ...
  },
  {
    ...
    pattern: /^(markdown-text|MarkdownPrimitive)-.*\.js$/,
...
function resolveBudgetResult(files, budget) {
  ...
  if (!matchedFile) {
    return {
      label: budget.label,
      status: 'missing',
      details: `No asset matched ${budget.pattern}`,
    }
  }
...
function writeReports(report) {
  ...
  writeFileSync(
    join(REPORT_DIR.pathname, 'build-metrics.json'),
  ...
  const markdownLines = [
    ...
    '| asset | raw | gzip |',
    '| --- | ---: | ---: |',
    ...report.topAssets.map((asset) => `| ${asset.name} | ${formatKiB(asset.rawBytes)} | ${formatKiB(asset.gzipBytes)} |`),
    '',
    '## Budget Results',
    '',
    '| label | status | file | details |',
    '| --- | --- | --- | --- |',
    ...report.budgetResults.map((result) => `| ${result.label} | ${result.status} | ${result.file ?? '-'} | ${result.details} |`),
    '',
  ]
