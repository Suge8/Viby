...
describe('pairingTiming', () => {
    ...
    it('keeps client/desktop detection windows below the server-side session grace', () => {
        ...
        expect(PAIRING_TUNNEL_READY_TIMEOUT_MS).toBeLessThan(PAIRING_PEER_DISCONNECT_CLIENT_GRACE_MS)
        expect(PAIRING_LINK_SAMPLE_STALE_MS).toBeGreaterThan(PAIRING_STATS_POLL_INTERVAL_MS)
        expect(PAIRING_LINK_SAMPLE_STALE_MS).toBeLessThan(PAIRING_PEER_HEARTBEAT_INTERVAL_MS * 2)
    })
})
